...
static NSString *utteranceString = @" ";

static void getAndSpeakCompletion(void) {
    ...
    AVSpeechSynthesisVoice *voice = [AVSpeechSynthesisVoice voiceWithLanguage:@"es-US"];
...
NSLog(@"Would you like to set up a new API Key(@'Y/N?') \n");
    ...
    while (YES) {
      ...
      static NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (error) {
                ...
                for (NSDictionary *completion in completions) {
                    ...
                    if(text != nil) {
                        ...
                        utteranceString = text;
                        getAndSpeakCompletion();
                        NSLog(@"\n%@", utteranceString);
                    }
                }
                }
    ...
    return 0;
} //end of main.  That wasn't so hard. (Except now it is all outdated lol)
